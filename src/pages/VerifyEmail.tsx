...
import { ShieldCheck, Loader2, Sparkles, ArrowRight, RefreshCw, Mail } from "lucide-react";
...
export default function VerifyEmail() {
  ...
  const handleVerify = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("🎉 Email verified successfully! Welcome to SiteFlow AI.");
      navigate("/builder");
    } catch (error: unknown) {
        ...
        <div className="w-full max-w-md bg-card/80 border border-primary/20 backdrop-blur-2xl rounded-3xl p-8 shadow-2xl space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground max-w-[280px] mx-auto">
              Please enter the 6-digit verification code sent to your email address to activate your account.
            </p>
          </div>
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            ...
            >
              {isLoading ? (
                ...
                <>
                  Verify Email & Continue <ArrowRight className="ml-2 h-5 w-5" />
                </>
              )}
            </Button>
          </form>
