...
function inferImageCategory(text: string, businessType?: string): ImageCategory {
  // First try exact match on businessType from user input
  if (businessType) {
    const bt = businessType.toLowerCase();
    const directMatch = CATEGORY_MATCHERS.find(({ keywords }) =>
      keywords.some((k) => bt.includes(k) || k.includes(bt))
    );
    if (directMatch) return directMatch.category;
  }

  // Then try matching from HTML content
...
function sanitizeGeneratedHtml(html: string, businessType?: string): string {
  if (typeof DOMParser === "undefined") return html;

  const parser = new DOMParser();
  const doc = parser.parseFromString(html, "text/html");
  const category = inferImageCategory(doc.documentElement.outerHTML, businessType);
...
export function extractHtmlFromMessage(content: string, businessType?: string): string | null {
  const match = content.match(/```html\s*\n([\s\S]*?)```/);
  return match ? sanitizeGeneratedHtml(match[1].trim(), businessType) : null;
}
