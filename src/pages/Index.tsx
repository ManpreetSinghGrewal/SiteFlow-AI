...
const Index = () => {
  const {
    conversations, activeConversation, activeId, isLoading,
    setActiveId, createConversation, deleteConversation, sendMessage, stopGeneration,
  } = useChat();
  ...
  useEffect(() => {
    if (latestHtml && !isLoading) setPreviewHtml(latestHtml);
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      <ChatSidebar
        ...
        onSelect={setActiveId} onCreate={createConversation}
        onDelete={deleteConversation} open={sidebarOpen} onClose={() => setSidebarOpen(false)}
      />
          ...
          <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
            ...
            {messages.map((msg, i) => (
              <ChatMessageComponent
                key={i} message={msg} isLast={i === messages.length - 1}
                isLoading={isLoading && i === messages.length - 1}
                onPreview={(html) => setPreviewHtml(html)}
                onRegenerate={msg.role === "assistant" && !isLoading ? () => handleRegenerate(i) : undefined}
              />
            ))}
      ...
      {previewHtml && (
        <div className="hidden md:flex w-1/2 flex-shrink-0">
          <WebsitePreview html={previewHtml} onClose={() => setPreviewHtml(null)} />
        </div>
      )}
    </div>
  );
